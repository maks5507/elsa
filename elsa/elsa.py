    ...
    def summarize(self, text: str, factor: float = 0.5, use_stem: bool = False, use_lemm: bool = False, 
        ...
        for i in range(len(sentences)):
            if i in filtered_sentences_set:
                ...
                sentences_scores += [0]
        summary = self.abstractive_model([sentences], [sentences_scores], use_gpu=False, **abstractive_params)
        return summary

    def gen_sentences_scores(self, text: str, factor: float = 0.5,
                             use_stem: bool = False,
                             use_lemm: bool = False,
                             check_stopwords: bool = True,
                             check_length: bool = True) -> str:
        cf_text = self.coreference_resolution.resolve(text)
        ...
        for i in range(len(sentences)):
            if i in filtered_sentences_set:
                ...
                sentences_scores += [0]
        return sentences, sentences_scores

    def abstractive_summarize(self, batch_sentences, batch_sentences_scores, **abstractive_params) -> str:
        batch_summary = self.abstractive_model(batch_sentences, batch_sentences_scores, use_gpu=True, **abstractive_params)
